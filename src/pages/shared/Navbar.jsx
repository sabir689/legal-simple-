...
const Navbar = () => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-[1000] transition-all duration-500 ${isScrolled || isOpen
      ...
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
            ...
            {user ? (
              <div className="flex items-center gap-4 group relative">
                <div className="text-right hidden lg:block">
                  <p className="text-[9px] font-black uppercase tracking-widest text-[#4f46e5] leading-none mb-1">Active Session</p>
                ...
                {/* 2. DESKTOP DROPDOWN OPTIONS */}
                <div className="absolute top-full right-0 mt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 translate-y-2 group-hover:translate-y-0 min-w-[140px] bg-white dark:bg-[#0a0a0a] border border-zinc-100 dark:border-white/5 shadow-2xl p-2">
                  <Link to="/dashboard" className="block w-full text-left bg-zinc-50 dark:bg-white/5 text-zinc-900 dark:text-white text-[9px] font-black uppercase tracking-widest px-4 py-3 mb-1 hover:bg-[#4f46e5] hover:text-white transition-colors">
                    Dashboard
                  </Link>
                  <button
                    onClick={handleLogout}
                    className="block w-full text-left bg-red-500/5 text-red-500 text-[9px] font-black uppercase tracking-[0.2em] px-4 py-3 hover:bg-red-500 hover:text-white transition-all"
                  >
                    Log Out
                  </button>
                </div>
        ...
        >
          <div className="flex flex-col gap-1.5 items-end">
            <Motion.span animate={isOpen ? { rotate: 45, y: 8, width: 28 } : { rotate: 0, y: 0, width: 28 }} className="h-0.5 bg-zinc-900 dark:bg-white block origin-center" />
            <Motion.span animate={isOpen ? { opacity: 0 } : { opacity: 1 }} className="h-0.5 w-5 bg-[#4f46e5] block" />
            <Motion.span animate={isOpen ? { rotate: -45, y: -8, width: 28 } : { rotate: 0, y: 0, width: 28 }} className="h-0.5 bg-zinc-900 dark:bg-white block origin-center" />
          </div>
        </button>
      </div>
        ...
        {isOpen && (
          ...
          >
            <div className="flex flex-col gap-8">
              ...
              {/* 3. MOBILE DASHBOARD OPTION (Top of list) */}
              {user && (
                <Link
                  to="/dashboard"
                  onClick={closeMenu}
                  className="text-5xl font-black uppercase tracking-tighter text-[#4f46e5] italic transition-all"
                >
                  Dashboard
                </Link>
              )}
              ...
              {user ? (
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <img
                      src={user.photoURL || `https://ui-avatars.com/api/?name=${user.displayName || 'User'}&background=4f46e5&color=fff`}
                      className="w-12 h-12 rounded-xl border-2 border-[#4f46e5]"
                      alt="User"
                    />
                    <div>
                      <p className="text-xs font-black dark:text-white uppercase truncate max-w-[150px]">{user.displayName}</p>
                      <p className="text-[10px] text-[#4f46e5] font-black uppercase tracking-widest leading-none">Access: Pro</p>
                    </div>
                  </div>
                  <button
                    onClick={handleLogout}
